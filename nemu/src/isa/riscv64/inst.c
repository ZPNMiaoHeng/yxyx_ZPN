...
#define INSTPAT_MATCH(s, name, type, ... /* body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 111 ????? 11000 11", bgeu   , B, if((uint32_t)src1 >= (uint32_t)src2) s->dnpc = s->pc + dest);
  ...
  INSTPAT("0000000 ????? ????? 001 ????? 00110 11", slliw  , I, int i=BITS(src2,5,5);printf("%d/n",i); if(i == 0) R(dest) = SEXT(BITS((src1 << BITS(src2,4,0)),31,0),32));
  INSTPAT("000000? ????? ????? 101 ????? 00100 11", srli   , I, R(dest) = src1 >> BITS(src2,5,0));
  INSTPAT("0000000 ????? ????? 101 ????? 00110 11", srliw  , I, int i=BITS(src2,5,5);printf("%d/n",i); if(i == 0) R(dest) = SEXT(BITS(((unsigned)src1 >> src2),31,0),32));
  ...
  INSTPAT("010000? ????? ????? 101 ????? 00110 11", sraiw  , I, int i=BITS(src2,5,5);printf("%d/n",i); if(i == 0) R(dest) = SEXT((signed)BITS(src1,31,0) >> BITS(src2,5,0),32));    //doubt  suanshuyiwei
  ...
  INSTPAT("0000000 ????? ????? 101 ????? 01110 11", srlw   , R, R(dest) = SEXT((unsigned)(BITS(src1,31,0) >> src2),32)); 
  ...
  INSTPAT("??????? ????? ????? 010 ????? 00000 11", lw     , I, R(dest) = SEXT(Mr(src1 + src2, 4),32));                                //yes
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, R(dest) = s->pc + (word_t)4;s->dnpc = s->pc + src1);
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, int t = s->pc + (word_t)4;s->dnpc = (src1 + src2) & ~(word_t)1;R(dest) = t);
