...
/**
  ...
  */
    ...
    class ALU extends Module {
      ...
      adder.io.Cin  := io.ALUCtr(3)
      bShifter.io.Din   := io.Asrc
      bShifter.io.Shamt := io.Bsrc(6, 0)
      bShifter.io.LR    := LR
      bShifter.io.AL    := AL

      val SubAdd = Mux((io.ALUCtr === "b1000".U || io.ALUCtr === "b0010".U || io.ALUCtr === "b1010".U), 1.U, 0.U)
      val US     = Mux((io.ALUCtr === "b0010".U), 1.U, 0.U) //带符号数



      val xor = io.Asrc ^ io.Bsrc
      val or  = io.Asrc | io.Bsrc
      val and = io.Asrc & io.Bsrc
      val less = Mux(io.ALUCtr(3) === 1.U, (adder.io.Carry ^ SubAdd), (adder.io.Overflow ^ adder.io.Result))
    ...
    io.Result := MuxCase(0.U, Array(
      ...
      (io.ALUCtr(2, 0) === "b010".U) -> less,                // 做减法，选择带符号小于置位结果输出, Less按带符号结果设置
      (io.ALUCtr(2, 0) === "b011".U) -> io.Bsrc,                          // 选择ALU输入B的结果直接输出
      (io.ALUCtr(2, 0) === "b100".U) -> xor,                // 异或输出
      (io.ALUCtr(2, 0) === "b101".U) -> bShifter.io.Dout,                // 移位器输出，逻辑右移
      (io.ALUCtr(2, 0) === "b110".U) -> or,                // 逻辑或输出
      (io.ALUCtr(2, 0) === "b111".U) -> and                // 逻辑与输出                                        
    ))
